...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, the earlier post-completion UI refactor program remains complete through `UI-13`, and the post-completion MUI refactor program is now fully complete through `MUI-14` on `feature/mui-refactor-foundation`. The durable docs now describe Material UI plus the app-owned wrapper layer as the canonical UI runtime, the frozen `src/app/tailwind-freeze.css` stylesheet as a compatibility layer only, and the supported compose test workflow as an isolated persistent `onesource-test` stack.
- Next recommended item: no remaining MUI checklist items. Await the next user-directed follow-up outside the completed MUI epic.
- Blockers: no product blocker is open. `make compose-test-e2e` still begins by running `docker compose down --remove-orphans`, so it tears down any user-kept live `web`/`worker` environment before the isolated browser run; restore it afterward with `docker compose run --rm --build web npx prisma migrate deploy`, `docker compose run --rm --build web npm run db:seed`, `docker compose up --build -d web worker`, and `curl -fsS http://127.0.0.1:3000/api/health`. The compose test database still stays internal to the compose network instead of publishing host port `5432`, which avoids collisions with unrelated local PostgreSQL containers. Closing the matching GitHub issue may still fail with `403 Resource not accessible by integration`; issue-close attempts for `#5` through `#13` have all hit that exact connector-permission limit, so `#14` may require the same fallback.
- Files touched in latest loop: `PRD.md`, `README.md`, `docs/architecture.md`, `docs/runbook.md`, and `NOTES.md`.
- Tests run in latest loop: `make compose-test-lint`; `make compose-test`; `make compose-test-build`; `make compose-test-browser-image-fresh`; `make compose-test-e2e`; `git diff --check`; and the documented default-stack restore commands (`docker compose run --rm --build web npx prisma migrate deploy`; `docker compose run --rm --build web npm run db:seed`; `docker compose up --build -d web worker`; `curl -fsS http://127.0.0.1:3000/api/health`). Verification used `docker compose` and Chromium Playwright; all required checks passed for `MUI-14`, including the mobile drawer smoke and the tablet-sized horizontal-overflow sweep. `make clean-dev-artifacts` remains intentionally skipped because the user asked to keep the environment available on the feature branch.
