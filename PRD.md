...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, and the earlier post-completion UI refactor program remains complete through `UI-13`. The post-completion MUI refactor program is now complete through `MUI-08` on `feature/mui-refactor-foundation`: the full opportunities route family now renders through the shared MUI foundation, including the list shell, filter rail, preview panel actions, create and edit forms, workspace header and section navigation, and the task, milestone, note, document, proposal, bid-decision, stage-transition, and closeout manager surfaces. A production-only `/opportunities` crash caused by MUI `FormControl` child introspection inside the shared field wrapper was fixed by simplifying `FormField` to a label plus helper-text stack while preserving the same accessibility wiring. The required compose-backed verification plus Chromium smoke coverage is green.
- Next recommended item: `MUI-09 Migrate the /sources discovery and import workspace onto the MUI system.`
- Blockers: no blocker prevents starting `MUI-09`. The compose browser-path trap still applies: after route-level or smoke-spec edits, explicitly rebuild `web` and `playwright` with `docker compose -f docker-compose.test.yml build --no-cache web playwright` before rerunning `make compose-test-e2e`, otherwise Chromium can reuse a stale app bundle. The direct `docker compose -f docker-compose.test.yml run --rm --build test ...` commands and the `make compose-test-e2e` target still share the default compose project name, so they can recreate `db` or tear down a user-kept live `web`/`worker` stack; if the live environment must remain available afterward, restore it with `docker compose run --rm --build web npx prisma migrate deploy`, `docker compose run --rm --build web npm run db:seed`, `docker compose up --build -d web worker`, and `curl -fsS http://127.0.0.1:3000/api/health`. The compose test database still stays internal to the compose network instead of publishing host port `5432`, which avoids collisions with unrelated local PostgreSQL containers. Closing the matching GitHub issue may still fail with `403 Resource not accessible by integration`; the earlier issue-close attempts for `#5`, `#6`, and `#7` hit that exact connector-permission limit, so the same limitation may affect `#8` even after the branch push.
- Files touched in latest loop: `src/components/opportunities/opportunity-bid-decision-manager.tsx`, `src/components/opportunities/opportunity-closeout-manager.tsx`, `src/components/opportunities/opportunity-document-manager.tsx`, `src/components/opportunities/opportunity-form.tsx`, `src/components/opportunities/opportunity-list.test.tsx`, `src/components/opportunities/opportunity-list.tsx`, `src/components/opportunities/opportunity-milestone-manager.tsx`, `src/components/opportunities/opportunity-note-manager.tsx`, `src/components/opportunities/opportunity-proposal-manager.tsx`, `src/components/opportunities/opportunity-stage-transition-panel.tsx`, `src/components/opportunities/opportunity-task-manager.tsx`, `src/components/opportunities/opportunity-workspace.tsx`, `src/components/ui/button.tsx`, `src/components/ui/form-field.tsx`, `src/components/ui/ui-foundation.test.tsx`, `tests/smoke.spec.ts`, `PRD.md`, `README.md`, `docs/architecture.md`, and `NOTES.md`.
- Tests run in latest loop: focused `npm test -- src/components/opportunities/opportunity-list.test.tsx src/components/opportunities/opportunity-form.test.tsx src/components/opportunities/opportunity-workspace.test.tsx src/components/opportunities/opportunity-proposal-manager.test.tsx src/components/opportunities/opportunity-action-refresh.test.tsx`; focused `npm test -- src/components/ui/ui-foundation.test.tsx src/components/opportunities/opportunity-list.test.tsx`; local `npm run build` twice during diagnosis and closeout; `docker compose -f docker-compose.test.yml build --no-cache web playwright`; `make compose-test-e2e`; `docker compose -f docker-compose.test.yml run --rm --build test npm run lint`; `docker compose -f docker-compose.test.yml run --rm --build test npm test`; `docker compose -f docker-compose.test.yml run --rm --build test npm run build`; `git diff --check`; and the documented default-stack restore commands (`docker compose run --rm --build web npx prisma migrate deploy`; `docker compose run --rm --build web npm run db:seed`; `docker compose up --build -d web worker`; `curl -fsS http://127.0.0.1:3000/api/health`). Verification used `docker compose` and Chromium Playwright; all required checks passed for `MUI-08`. `make clean-dev-artifacts` remains intentionally skipped because the user asked to keep the environment available on the feature branch for continued work.
