...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, and the earlier post-completion UI refactor program remains complete through `UI-13`. The post-completion MUI refactor program is now complete through `MUI-04` on `feature/mui-refactor-foundation`: the public auth surfaces now share an MUI-backed public access shell, `/sign-in` and `/forbidden` no longer depend on the old Tailwind-first page framing, the sign-in form now exposes field-level validation and deterministic pending states through the shared MUI wrappers, and the required compose-backed verification plus Chromium smoke coverage is green.
- Next recommended item: `MUI-05 Standardize empty, loading, error, permission, and transient feedback patterns on the MUI system.`
- Blockers: no blocker prevents starting `MUI-05`. The compose browser-path trap still applies: after route-level or smoke-spec edits, explicitly rebuild `web` and `playwright` with `docker compose -f docker-compose.test.yml build --no-cache web playwright` before rerunning `make compose-test-e2e`, otherwise Chromium can reuse a stale app bundle. The additional operational note from `MUI-03` still applies as well: the direct `docker compose -f docker-compose.test.yml run --rm --build test ...` commands and the `make compose-test-e2e` target share the default compose project name, so they can recreate `db` or tear down a user-kept live `web`/`worker` stack; if the live environment must remain available afterward, restore it with the documented default-stack bootstrap and `docker compose up --build -d web worker`.
- Files touched in latest loop: `src/app/forbidden/page.tsx`, `src/app/sign-in/page.tsx`, `src/components/auth/public-access-shell.tsx`, `src/components/auth/public-access-shell.test.tsx`, `src/components/auth/sign-in-form.tsx`, `src/components/auth/sign-in-form.test.tsx`, `src/components/ui/button.tsx`, `tests/smoke.spec.ts`, `PRD.md`, `README.md`, `docs/architecture.md`, and `NOTES.md`.
- Tests run in latest loop: live Playwright CLI review against `http://127.0.0.1:3000/sign-in` and `http://127.0.0.1:3000/forbidden?permission=workspace_settings`; focused `docker compose -f docker-compose.test.yml run --rm --build test npm test -- src/components/auth/public-access-shell.test.tsx src/components/auth/sign-in-form.test.tsx`; full `docker compose -f docker-compose.test.yml run --rm --build test npm run lint`; full `docker compose -f docker-compose.test.yml run --rm --build test npm test`; full `docker compose -f docker-compose.test.yml run --rm --build test npm run build`; explicit `docker compose -f docker-compose.test.yml build --no-cache web playwright`; `make compose-test-e2e`; and `git diff --check`. Verification used `docker compose` and Chromium Playwright; all required checks passed for `MUI-04`. `make clean-dev-artifacts` remains intentionally skipped because the user asked to keep the environment available on the feature branch for continued work.
