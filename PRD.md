...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, the earlier post-completion UI refactor program remains complete through `UI-13`, and the post-completion MUI follow-ups now remain complete through `MUI-19` on `feature/mui-refactor-foundation`. `MUI-19` splits the oversized settings surface into an ops-focused `/settings` page plus a dedicated `/settings/users` workspace, adds a `Users & Roles` admin-rail destination, introduces audit-backed invite or role-update or disable or re-enable flows over a MUI `DataGrid` workspace, and tightens the shared dense-table contract so `DataTable`, `TableSkeleton`, and `MuiDataGrid` use a lighter radius without clipping text or focus treatment at the shell edge.
- Next recommended item: continue the semantic reset with another route-family slice such as `MUI-20` for `/sources`, `/analytics`, or the remaining hybrid admin/detail surfaces so the page composition and information density catch up with the new shell baseline.
- Blockers: no product blocker is open. Host-side verification on this machine still hits generated-artifact ownership drift between Docker-managed directories and direct local tooling: before host-side lint the loop may need a temporary Docker `chown -R 1000:1000 /workspace/.docker` helper so `eslint .` can traverse the repo, while browser verification still requires restoring `.docker/postgres-data` to Postgres ownership (`999`) and sometimes waiting for the restarted container to become reachable before `npm run db:seed` succeeds. The user-kept live stack on `3000` should remain untouched; use a temporary built server on `127.0.0.1:3002` with `SAM_GOV_USE_FIXTURES=true` for deterministic local Chromium verification because a second `next dev` instance cannot run against the same repo while the kept dev server is already active. The serialized local Playwright smoke suite still mutates shared seeded data, so rerunning `npm run e2e` after a partial failure still requires `npm run db:seed` first. During the full temporary-server smoke run, the app can still log the pre-existing seeded document download `ENOENT` errors under `.data/opportunity-documents/...`; the Chromium suite may still pass, but the missing seeded files remain an operational cleanup item outside this ticket. `make clean-dev-artifacts` remains intentionally skipped because the user asked to keep the environment available.
- Files touched in latest loop: `NOTES.md`, `PRD.md`, `README.md`, `docs/architecture.md`, `docs/testing.md`, `src/app/(app)/settings/page.tsx`, `src/app/(app)/settings/users/page.tsx`, `src/app/(app)/settings/users/actions.ts`, `src/components/admin/admin-console.tsx`, `src/components/admin/admin-console.test.tsx`, `src/components/admin/admin-shared.tsx`, `src/components/admin/admin-user-management.tsx`, `src/components/admin/admin-user-management.test.tsx`, `src/components/layout/authenticated-app-shell.tsx`, `src/components/layout/authenticated-app-shell.test.tsx`, `src/components/ui/data-table.tsx`, `src/components/ui/table-skeleton.tsx`, `src/components/ui/ui-interactions.test.tsx`, `src/modules/admin/admin.repository.ts`, `src/modules/admin/admin.repository.test.ts`, `src/modules/admin/admin.types.ts`, `src/modules/admin/user-management-form.schema.ts`, `src/modules/admin/user-management.service.ts`, `src/modules/admin/user-management.service.test.ts`, `src/modules/audit/audit.service.ts`, `src/theme/onesource-theme.ts`, `src/theme/onesource-theme.test.ts`, and `tests/smoke.spec.ts`.
- Tests run in latest loop: focused `npx eslint src/components/ui/data-table.tsx src/components/ui/table-skeleton.tsx src/components/ui/ui-interactions.test.tsx src/components/admin/admin-user-management.tsx src/theme/onesource-theme.ts src/theme/onesource-theme.test.ts`; focused `npx vitest run src/components/ui/ui-interactions.test.tsx src/theme/onesource-theme.test.ts`; repeated full local `npm run lint`; repeated full local `npm test`; repeated full local `npm run build`; repeated `npm run db:seed`; repeated focused `npx eslint tests/smoke.spec.ts` while stabilizing the browser contract for the new user-management flow; final full local Chromium smoke via `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3002 npm run e2e` against a temporary fixture-backed built server; and `git diff --check`. Host-side lint required the documented temporary Docker `chown -R 1000:1000 /workspace/.docker` helper, browser verification required restoring `.docker/postgres-data` to `999:999`, restarting `db` plus `worker`, and occasionally retrying `npm run db:seed` after the restarted Postgres container moved from `health: starting` to ready. The temporary `3002` verification server was shut down after the final green browser pass; the user-kept main stack was left running.
