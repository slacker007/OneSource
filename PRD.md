...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, and the earlier post-completion UI refactor program remains complete through `UI-13`. The post-completion MUI refactor program is now complete through `MUI-03` on `feature/mui-refactor-foundation`: the authenticated shell, grouped navigation, command center, notifications surface, and mobile drawer now render through the MUI wrapper layer, the shell opener actions blur focus before opening modal surfaces to avoid the browser `aria-hidden` accessibility warning seen during live review, and the full required verification path for the user-facing shell item is green.
- Next recommended item: `MUI-04 Rebuild the public auth surfaces on Material UI form and feedback patterns.`
- Blockers: no blocker prevents starting `MUI-04`. The compose browser-path trap still applies: after route-level or smoke-spec edits, explicitly rebuild `web` and `playwright` with `docker compose -f docker-compose.test.yml build --no-cache web playwright` before rerunning `make compose-test-e2e`, otherwise Chromium can reuse a stale app bundle. One additional operational note from this loop is now documented in `docs/testing.md`: the direct `docker compose -f docker-compose.test.yml run --rm --build test ...` commands and the `make compose-test-e2e` target share the default compose project name, so they can recreate `db` or tear down a user-kept live `web`/`worker` stack; if the live environment must remain available afterward, restore it with the documented default-stack bootstrap and `docker compose up --build -d web worker`.
- Files touched in latest loop: `src/components/layout/authenticated-app-shell.tsx`, `src/components/layout/authenticated-app-shell.test.tsx`, `PRD.md`, `README.md`, `docs/testing.md`, and `NOTES.md`.
- Tests run in latest loop: live Playwright CLI shell review against `http://127.0.0.1:3000/`; focused `docker compose -f docker-compose.test.yml run --rm --build test npm test -- src/components/layout/authenticated-app-shell.test.tsx`; `docker compose run --rm --build web npx prisma migrate deploy`; `docker compose run --rm --build web npm run db:seed`; full `docker compose -f docker-compose.test.yml run --rm --build test npm run lint`; full `docker compose -f docker-compose.test.yml run --rm --build test npm test`; full `docker compose -f docker-compose.test.yml run --rm --build test npm run build`; explicit `docker compose -f docker-compose.test.yml build --no-cache web playwright`; `make compose-test-e2e`; and `git diff --check`. Verification used `docker compose` and Chromium Playwright; all required checks passed for `MUI-03`. `make clean-dev-artifacts` remains intentionally skipped because the user asked to keep the environment available on the feature branch for continued work.
