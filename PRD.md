...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, the earlier post-completion UI refactor program remains complete through `UI-13`, and the post-completion MUI follow-ups now remain complete through `MUI-17` on `feature/mui-refactor-foundation`. `MUI-17` starts the semantic design reset the user requested: the OneSource theme now uses a compact neutral-first baseline, the frozen compatibility variables now inherit that same baseline through `src/app/design-reset.css`, the authenticated shell no longer depends on the older warm green palette for its header, command center, notifications, or workbench states, and the dashboard has been rebuilt into a cleaner MUI-only operational layout under the new typography and spacing contract. The repo-level formatting contract is also truthful again because `.prettierrc.json` no longer points at the removed Tailwind plugin.
- Next recommended item: continue the semantic reset with the next route-family slice, ideally a new `MUI-18` for one of the remaining utility-heavy authenticated surfaces such as `/sources`, `/analytics`, or `/settings`, so the route-level layout composition catches up with the new shell and dashboard baseline.
...
- Files touched in latest loop: `.prettierrc.json`, `PRD.md`, `README.md`, `NOTES.md`, `docs/architecture.md`, `src/app/design-reset.css`, `src/app/layout.tsx`, `src/components/home/app-shell-preview.test.tsx`, `src/components/home/dashboard-landing.tsx`, `src/components/layout/app-theme-provider.test.tsx`, `src/components/layout/authenticated-app-shell.tsx`, `src/components/layout/authenticated-app-shell.test.tsx`, `src/components/ui/badge.tsx`, `src/lib/tooling/tailwind-removal.test.ts`, `src/theme/onesource-theme.test.ts`, `src/theme/onesource-theme.ts`, and `tests/smoke.spec.ts`.
- Tests run in latest loop: focused `npx vitest run src/theme/onesource-theme.test.ts src/components/home/app-shell-preview.test.tsx src/components/layout/authenticated-app-shell.test.tsx`; full local `npm run lint`; full local `npm test`; full local `npm run build`; `npm run db:seed`; full local Chromium smoke via `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3002 npm run e2e` against a temporary fixture-backed built server; and `git diff --check`. Host-side lint required the documented temporary Docker `chown -R 1000:1000 /workspace/.docker` helper, browser verification required restoring `.docker/postgres-data` to `999:999` and restarting `db` plus `worker`, and the temporary `3002` server was shut down after the browser pass. `make clean-dev-artifacts` remains intentionally skipped because the user asked to keep the environment available.
