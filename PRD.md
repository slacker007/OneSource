...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, the earlier post-completion UI refactor program remains complete through `UI-13`, and the post-completion MUI refactor program remains complete through `MUI-14` on `feature/mui-refactor-foundation`. The broader post-`MUI-14` hardening worktree is still in progress and now includes a dashboard-specific visual/usability pass: the shared OneSource MUI theme exposes a fuller semantic token system, the wrapper primitives now consume that theme directly, the select wrapper is now a real MUI `Select`/`MenuItem`, the authenticated shell uses the persistent MUI drawer rail with the refined workspace identity and denser user summary, and the dashboard route has been re-audited in headed Chromium and tightened so the redundant page hero is gone, the intro is compact, the KPI strip is readable, and the attention queue plus top pursuits split correctly at common laptop-desktop widths after correcting the dashboard breakpoints from `xl` to `lg`. The earlier user-reported `/opportunities` local-runtime break also remains fixed: the shared select wrapper no longer injects a custom `input={<OutlinedInput ... />}` element, which had been crashing MUI `Select` with `Cannot read properties of undefined (reading 'inputProps')`.
- Next recommended item: continue the user-directed route-by-route visual hardening pass outside the completed MUI epic, starting with the next highest-friction authenticated route after the dashboard (likely `/settings`, which still needs information-architecture cleanup more than styling cleanup).
- Blockers: no product blocker is open. `make compose-test-e2e` still begins by running `docker compose down --remove-orphans`, so it tears down any user-kept live `web`/`worker` environment before the isolated browser run; restore it afterward with `docker compose run --rm --build web npx prisma migrate deploy`, `docker compose run --rm --build web npm run db:seed`, `docker compose up --build -d web worker`, and `curl -fsS http://127.0.0.1:3000/api/health`. The compose test database still stays internal to the compose network instead of publishing host port `5432`, which avoids collisions with unrelated local PostgreSQL containers. Closing the matching GitHub issue may still fail with `403 Resource not accessible by integration`; issue-close attempts for `#5` through `#13` have all hit that exact connector-permission limit, so any later issue-close attempt for this follow-up may require the same fallback. The serialized local Playwright smoke suite mutates shared seeded data, so rerunning `npm run e2e` after a partial failure still requires `npm run db:seed` first. When a user-kept `next dev` server is already running on `3000`, deterministic local Playwright verification on a second port cannot use another `next dev` instance for the same repo because Next.js refuses the duplicate dev server; use a built server (`next start` or the standalone output) for the temporary fixture-backed verification target instead.
- Files touched in latest loop: the broader uncommitted UI-hardening worktree still includes `README.md`, `docs/architecture.md`, `src/components/admin/admin-console.tsx`, `src/components/layout/authenticated-app-shell.tsx`, `src/components/layout/authenticated-app-shell.test.tsx`, `src/components/ui/select.tsx`, `src/components/ui/ui-foundation.test.tsx`, `src/theme/onesource-theme.ts`, `src/theme/onesource-theme.test.ts`, and other related follow-up files from the ongoing post-`MUI-14` pass; the latest dashboard-specific loop changed `PRD.md`, `NOTES.md`, `src/app/(app)/page.tsx`, `src/components/home/dashboard-landing.tsx`, `src/components/home/app-shell-preview.test.tsx`, and `tests/smoke.spec.ts`.
- Tests run in latest loop: focused local `npm run lint -- src/app/(app)/page.tsx src/components/home/dashboard-landing.tsx src/components/home/app-shell-preview.test.tsx`; focused local `npm test -- src/components/home/app-shell-preview.test.tsx`; local `npm run build`; full local `npm run lint`; full local `npm test`; `npm run db:seed`; full local Chromium smoke via `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3001 npm run e2e` against a temporary fixture-backed local server; `git diff --check`; and headed Playwright CLI visual inspection of the live dashboard on `http://127.0.0.1:3000` before and after the fix, with the inspection browser explicitly closed afterward. The local smoke run also forced two stale dashboard assertions in `tests/smoke.spec.ts` to be updated from the retired `Capture command center` heading to the new dashboard intro heading so the regression suite matches the current UI contract. `make clean-dev-artifacts` remains intentionally skipped because the user asked to keep the environment available on the feature branch.
