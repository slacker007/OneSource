...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, and `UI-01` through `UI-08` now remain complete. `UI-08` is closed: the `/sources` route now renders as a denser discovery workspace with connector tabs, connector-scoped saved-search visibility, collapsible advanced filters, active-filter chips, compact result scanning, inline import-state messaging on the selected result row, a right-rail import preview, and translated-query inspection while preserving the existing guarded search, preview, link, create, and CSV import flows. The one-time project completion marker remains the earlier `2026-04-18T17:54:41Z` emission and should not be re-emitted until the full UI refactor program is actually complete.
- Next recommended item: `UI-09 Redesign the task surfaces for triage-first execution across My Tasks, Team Tasks, Calendar, and Kanban views while preserving task CRUD, reminder state, and opportunity linkage.`
- Blockers: no blocker currently prevents `UI-09`. Current environment limitation, not blocker: `make compose-up-detached` still fails in this environment with a Docker build-context `changes out of order` error before the app containers finish building, so host-run production verification may still require a compose-managed PostgreSQL service plus a separately started Next.js server. Current environment limitation, not blocker: when Playwright targets an already running host-side app with `PLAYWRIGHT_BASE_URL`, the running app must also be started with `SAM_GOV_USE_FIXTURES=true` for deterministic `/sources` smoke coverage because Playwright does not inject fixture mode into an externally managed server. Current limitation, not blocker: the decision-console effort metric intentionally uses tracked execution artifacts as a proxy because the schema does not yet persist hours or labor-cost fields. Current limitation, not blocker: scheduled sync execution is only implemented for saved searches whose `sourceSystem` is `sam_gov`, so the seeded future-connector search still logs a warning and remains non-executable until those connectors land. Current limitation, not blocker: the CRM, document-repository, and communication adapters are deterministic dry runs only, so no live outbound credentials, webhook validation, or callback signatures are in scope yet. Current limitation, not blocker: compose builds still emit Prisma OpenSSL detection warnings inside the slim Node image even when build and runtime verification pass. Current environment limitation, not blocker: the serialized Playwright smoke suite mutates one shared seeded database, so reruns in the same loop should begin with `npm run db:seed` before `npm run e2e` or `make compose-test-e2e` if prior smoke scenarios already changed seeded proposal, task, or closeout records. Current environment limitation, not blocker: after a full local database reset, rerun `npx prisma migrate deploy` and `npm run db:seed` before `make compose-test-e2e` because the compose browser target expects a migrated and seeded database rather than bootstrapping one itself. Current environment limitation, not blocker: this machine can still surface intermittent PostgreSQL storage faults such as `55000 attempted to delete invisible tuple`, `XX002 ... unexpected zero page`, or `XX000 unexpected data beyond EOF`; the verified recovery path remains `docker compose down -v --remove-orphans`, remove and recreate `.docker/postgres-data` plus `.data/opportunity-documents`, then rerun `docker compose up -d db`, `npx prisma migrate deploy`, and `npm run db:seed` before continuing verification. Unrelated repo sync blocker remains: the earlier local history rewrite still has not been force-pushed to `origin` from this environment because the remote is SSH-based and this runtime lacks an authenticated SSH path.
- Files touched in latest loop: `NOTES.md`, `PRD.md`, `README.md`, `docs/testing.md`, `src/components/sources/source-search.tsx`, `src/components/sources/source-search.test.tsx`, `src/modules/source-integrations/source-search.service.ts`, `src/modules/source-integrations/source-search.service.test.ts`, and `tests/smoke.spec.ts`.
- Tests run in latest loop: full recovery `docker compose down -v --remove-orphans`; remove and recreate `.docker/postgres-data` plus `.data/opportunity-documents`; full `docker compose up -d db`; full `npx prisma migrate deploy`; full `npm run db:seed`; full `make compose-test-e2e`; full `npm run prisma:validate`; full `npm run lint`; full `npm test`; full `npm run build`; and `git diff --check`. During the first compose browser attempt after the reset, Playwright stayed on `/sign-in` because the reset database had not yet been migrated or seeded; rerunning `npx prisma migrate deploy` plus `npm run db:seed` before `make compose-test-e2e` resolved that environment issue. `docker compose` was used for PostgreSQL and live Chromium verification, and Playwright Chromium ran against the compose-managed production app.
