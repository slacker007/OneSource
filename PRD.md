...
- Current status: `P0-01`, `P0-02`, `P0-02a`, `P0-03`, `P0-04`, `P1-01`, and `P1-02` are complete. This loop added the core opportunity and source-lineage schema, migration `20260417222634_add_opportunity_lineage_schema`, deterministic opportunity seed fixtures, and durable documentation updates for the new persistence baseline.
- Next recommended item: `P1-02a Add source-connector metadata and persistence models that support more than one external source without schema rewrites.`
- Blockers: None for `P1-02`. Raw Node HTTP fetches to the npm registry still fail in this environment, but `npm install` and the refreshed offline cache archive continue to support reproducible dependency and compose workflows.
- Files touched in latest loop: `NOTES.md`, `PRD.md`, `README.md`, `docs/architecture.md`, `docs/runbook.md`, `docs/security.md`, `docs/testing.md`, `prisma/schema.prisma`, `prisma/seed.mjs`, `prisma/opportunity-seed-scenarios.mjs`, `prisma/migrations/20260417222634_add_opportunity_lineage_schema/migration.sql`, and `src/lib/opportunities/opportunity-seed-scenarios.test.ts`.
- Tests run in latest loop: `npx prisma format`; `npm run prisma:validate`; `docker compose up -d db`; `npm run prisma:migrate:dev -- --name add_opportunity_lineage_schema`; `npm run db:seed`; `npm run lint`; `npm test`; `npm run build`; `npm run e2e`; `docker compose --profile test run --rm --build test run lint`; `docker compose --profile test run --rm --build test`; `docker compose --profile test run --rm --build test run build`; `docker compose --profile test up --build --abort-on-container-exit --exit-code-from playwright playwright`; `node --input-type=module -e "..."`; `docker compose down --remove-orphans`; and `git diff --check`. Verification used `docker compose` and Playwright.
