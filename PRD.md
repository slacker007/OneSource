...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, the earlier post-completion UI refactor program remains complete through `UI-13`, and the post-completion MUI follow-ups now remain complete through `MUI-18` on `feature/mui-refactor-foundation`. `MUI-18` rebuilds the authenticated left shell into a more traditional admin pattern on top of MUI: desktop now defaults to a mini persistent rail, collapsed groups expose hover or focus flyouts for sibling destinations, the expanded rail is tighter and more conventional, the old user-card plus workbench block are gone from the rail, and pinned or recent destinations now live only in the command center while the top bar keeps command search, alerts, identity, and sign-out. A final shell contrast follow-up in the same loop also fixed dark-on-dark rail labels by making the admin-rail text explicitly inherit the shell foreground color.
- Next recommended item: continue the semantic reset with a route-family slice such as `MUI-19` for `/sources`, `/analytics`, or `/settings` so the page composition and information density catch up with the new shell baseline.
- Blockers: no product blocker is open. Host-side verification on this machine still hits generated-artifact ownership drift between Docker-managed directories and direct local tooling: before host-side lint the loop may need a temporary Docker `chown -R 1000:1000 /workspace/.docker` helper so `eslint .` can traverse the repo, then browser verification still requires restoring `.docker/postgres-data` to Postgres ownership (`999`) and restarting `db` plus `worker` before `npm run db:seed` and Playwright will work again. The user-kept live stack on `3000` should remain untouched; use a temporary built server on `127.0.0.1:3002` with `SAM_GOV_USE_FIXTURES=true` for deterministic local Chromium verification because a second `next dev` instance cannot run against the same repo while the kept dev server is already active. The serialized local Playwright smoke suite still mutates shared seeded data, so rerunning `npm run e2e` after a partial failure still requires `npm run db:seed` first. During the full temporary-server smoke run, the app can still log the pre-existing seeded document download `ENOENT` errors under `.data/opportunity-documents/...`; the Chromium suite may still pass, but the missing seeded files remain an operational cleanup item outside this ticket. `make clean-dev-artifacts` remains intentionally skipped because the user asked to keep the environment available.
- Files touched in latest loop: `NOTES.md`, `PRD.md`, `README.md`, `docs/architecture.md`, `docs/testing.md`, `src/components/layout/authenticated-app-shell.tsx`, `src/components/layout/authenticated-app-shell.test.tsx`, and `tests/smoke.spec.ts`.
- Tests run in latest loop: focused `npx eslint src/components/layout/authenticated-app-shell.tsx src/components/layout/authenticated-app-shell.test.tsx tests/smoke.spec.ts`; focused `npx vitest run src/components/layout/authenticated-app-shell.test.tsx`; full local `npm run lint`; full local `npm test`; full local `npm run build`; repeated `npm run db:seed`; targeted Chromium reruns via `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3002 npx playwright test tests/smoke.spec.ts --project=chromium --grep "authenticated homepage smoke test|users can create and edit tracked opportunities from the app|users can open the opportunity workspace and review seeded sections"`, `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3002 npx playwright test tests/smoke.spec.ts --project=chromium --grep "authenticated homepage smoke test"`, `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3002 npx playwright test tests/smoke.spec.ts --project=chromium --grep "desktop shell exposes the mini admin rail, command utilities, and notifications"`, and `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3002 npx playwright test tests/smoke.spec.ts --project=chromium --grep "users can open the opportunity workspace and review seeded sections"` while tightening the live smoke contract; final full local Chromium smoke via `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3002 npm run e2e` against a temporary fixture-backed built server; focused `npx eslint tests/smoke.spec.ts`; and `git diff --check`. Host-side lint required the documented temporary Docker `chown -R 1000:1000 /workspace/.docker` helper, browser verification required restoring `.docker/postgres-data` to `999:999` and restarting `db` plus `worker`, and the temporary `3002` server was shut down after the final browser pass.
