...
- Current status: `P0-01`, `P0-02`, `P0-02a`, `P0-03`, `P0-04`, `P1-01`, `P1-02`, `P1-02a`, `P1-03`, `P1-04`, `P1-05`, `P2-01`, `P2-02`, `P2-03`, `P2-04`, `P3-01`, `P3-02`, `P3-03`, `P4-01`, `P4-01a`, `P4-01b`, `P4-02`, `P4-03`, `P4-04`, `P5-01`, `P5-02`, `P5-03`, `P5-04`, `P6-01`, `P6-02`, `P6-03`, `P6-04`, `P6-05`, `P7-01`, `P7-02`, `P7-03`, `P7-04`, and `P7-05` are complete. The worker now runs four idempotent sweeps on each interval: deadline reminders, scheduled `sam.gov` source sync for due saved searches, queued document parsing retries for supported local uploads, and deterministic scorecard recalculation for stale or missing-current opportunities. Text-like document uploads now persist a queued extraction state instead of blocking the request path, and the new one-shot job entrypoints provide operator-visible runbook commands for each sweep. The project as a whole still cannot be claimed complete in this loop because later items `P7-06` through `P10-04` remain unchecked.
- Next recommended item: `P7-06 Add source sync observability for connector health, last successful sync, rate-limit handling, and failed import review`.
- Blockers: no product blocker remains on `P7-05`; the credentialed live `/sources` run is explicitly deferred to post-project follow-on `FP-01`. Current limitation, not blocker: scheduled sync execution is only implemented for saved searches whose `sourceSystem` is `sam_gov`, so the seeded future-connector search still logs a warning and counts as a failed run until those connectors land. Unrelated repo sync blocker remains: the earlier local history rewrite still has not been force-pushed to `origin` from this environment because the remote is SSH-based and this runtime lacks an authenticated SSH path.
- Files touched in latest loop: `NOTES.md`, `PRD.md`, `README.md`, `docs/architecture.md`, `docs/runbook.md`, `docs/testing.md`, `.env.example`, `package.json`, `package-lock.json`, `scripts/job-runtime.ts`, `scripts/run-deadline-reminder-job.ts`, `scripts/run-source-sync-job.ts`, `scripts/run-document-parsing-job.ts`, `scripts/run-opportunity-scorecard-job.ts`, `scripts/worker.ts`, `src/app/(app)/opportunities/actions.ts`, `src/components/opportunities/opportunity-document-manager.tsx`, `src/lib/env.ts`, `src/lib/env.test.ts`, `src/modules/audit/audit.service.ts`, `src/modules/opportunities/opportunity-document-storage.ts`, `src/modules/opportunities/opportunity-document-storage.test.ts`, `src/modules/opportunities/opportunity-document-parsing-job.ts`, `src/modules/opportunities/opportunity-document-parsing-job.test.ts`, `src/modules/opportunities/opportunity-scorecard-job.ts`, `src/modules/opportunities/opportunity-scorecard-job.test.ts`, `src/modules/source-integrations/source-sync-job.ts`, `src/modules/source-integrations/source-sync-job.test.ts`, and `tests/smoke.spec.ts`.
- Tests run in latest loop: focused `P7-05` verification plus full host and compose-backed closeout verification. Commands run: `npm install`; `npx vitest run src/lib/env.test.ts src/modules/opportunities/opportunity-document-storage.test.ts src/modules/opportunities/opportunity-document-parsing-job.test.ts src/modules/opportunities/opportunity-scorecard-job.test.ts src/modules/source-integrations/source-sync-job.test.ts`; `docker compose up -d db`; `npm run prisma:validate`; `npm run db:seed`; `npm run lint`; `npm test`; `npm run build`; `npm run e2e`; `make compose-test-lint`; `make compose-test`; `make compose-test-build`; `make compose-test-e2e`; `SAM_GOV_USE_FIXTURES=true npm run job:source-sync`; `npm run job:document-parse`; `npm run job:scorecards`; `git diff --check`; and `make compose-down`. Verification used `docker compose` for the database and compose-backed lint, unit, build, and Playwright Chromium runs; host Playwright also ran against a live application instance. The manual one-shot job evidence showed one seeded `sam.gov` search sync success, one expected warning for a future non-`sam.gov` saved search, zero pending documents after reseed, and ten stale opportunity scorecards recalculated.
