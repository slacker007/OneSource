...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, and deferred follow-on `FP-01` remains complete. The latest loop fixed an authenticated `/sources` crash where a stale JWT-backed session could survive a reseed or user-status change and then violate the `source_search_executions.requested_by_user_id` foreign key during search persistence. Auth.js session refresh now revalidates the backing user row and redirects stale or disabled users back to `/sign-in` before authenticated page loads or write actions trust the actor context. The one-time project completion marker remains the earlier `2026-04-18T17:54:41Z` emission.
...
- Files touched in latest loop: `NOTES.md`, `PRD.md`, `docs/runbook.md`, `docs/security.md`, `src/lib/auth/authenticate-user.ts`, `src/lib/auth/auth-options.ts`, and `src/lib/auth/auth-options.test.ts`.
- Tests run in latest loop: focused `npm test -- src/lib/auth/auth-options.test.ts`; focused `npm test -- src/modules/source-integrations/source-search.service.test.ts`; `docker compose up -d db`; `npm run prisma:validate`; `npm run db:seed`; `npm run lint`; `npm test`; `npm run build`; `npm run db:seed`; `SAM_GOV_USE_FIXTURES=true npm run start -- --hostname 127.0.0.1 --port 3000`; `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 SAM_GOV_USE_FIXTURES=true npm run e2e`; and an ad hoc headless Playwright plus Prisma regression script that signed in as `admin@onesource.local`, flipped that user to `DISABLED`, revisited `/sources`, and confirmed a redirect to `/sign-in` instead of a route-level error. `docker compose` was used for PostgreSQL, and Playwright was used both through the standard Chromium smoke suite and through the ad hoc stale-session regression script against the live host-run production app.
