...
- Current status: `P0-01`, `P0-02`, `P0-02a`, `P0-03`, `P0-04`, `P1-01`, `P1-02`, `P1-02a`, `P1-03`, `P1-04`, `P1-05`, `P2-01`, `P2-02`, `P2-03`, `P2-04`, `P3-01`, `P3-02`, `P3-03`, `P4-01`, `P4-01a`, `P4-01b`, `P4-02`, `P4-03`, `P4-04`, `P5-01`, `P5-02`, `P5-03`, and `P5-04` are complete. This loop added additive deadline-reminder state on tasks and milestones, a real reminder sweep under `scripts/deadline-reminder-job.mjs`, a worker that persists reminder transitions plus activity and audit evidence, seeded overdue and upcoming reminder state, workspace plus personal-task-queue reminder badges, and a compose-cache archive fix that keeps the canonical Playwright workflow green.
- Next recommended item: `P6-01 Create the organization profile and scoring configuration models for capabilities, certifications, vehicles, and weighted criteria.`
- Blockers: No product blocker for `P6-01`. Unrelated repo sync blocker remains: the earlier local history rewrite still has not been force-pushed to `origin` from this environment because the remote is SSH-based and this runtime lacks an authenticated SSH path.
- Files touched in latest loop: `NOTES.md`, `PRD.md`, `README.md`, `.env.example`, `docker-compose.yml`, `docs/architecture.md`, `docs/runbook.md`, `docs/testing.md`, `prisma/schema.prisma`, `prisma/opportunity-seed-scenarios.mjs`, `prisma/seed.mjs`, `prisma/migrations/20260418060827_add_deadline_reminder_state/migration.sql`, `scripts/deadline-reminder-job.mjs`, `scripts/refresh-offline-prisma-client.mjs`, `scripts/worker.mjs`, `src/components/opportunities/opportunity-milestone-manager.tsx`, `src/components/opportunities/opportunity-task-manager.tsx`, `src/components/opportunities/opportunity-workspace.tsx`, `src/components/opportunities/opportunity-workspace.test.tsx`, `src/components/tasks/personal-task-board.tsx`, `src/components/tasks/personal-task-board.test.tsx`, `src/modules/opportunities/deadline-reminder-job.test.ts`, `src/modules/opportunities/opportunity.repository.ts`, `src/modules/opportunities/opportunity.repository.test.ts`, `src/modules/opportunities/opportunity.types.ts`, and `tests/smoke.spec.ts`.
- Tests run in latest loop: `docker compose up -d db`; `npm run prisma:validate`; `DEADLINE_REMINDER_LOOKAHEAD_DAYS=7 npm run prisma:migrate:dev -- --name add_deadline_reminder_state`; `npm test -- src/modules/opportunities/deadline-reminder-job.test.ts src/modules/opportunities/opportunity.repository.test.ts src/components/opportunities/opportunity-workspace.test.tsx src/components/tasks/personal-task-board.test.tsx`; `npm run lint`; `npm run build`; `npm run db:seed`; `npm test`; `npm run e2e`; `DEADLINE_REMINDER_LOOKAHEAD_DAYS=7 make compose-test-lint`; `DEADLINE_REMINDER_LOOKAHEAD_DAYS=7 make compose-test`; `DEADLINE_REMINDER_LOOKAHEAD_DAYS=7 make compose-test-build`; `DEADLINE_REMINDER_LOOKAHEAD_DAYS=7 make compose-test-e2e`; `DEADLINE_REMINDER_LOOKAHEAD_DAYS=7 make compose-down`; and `git diff --check`. Verification used Docker and Playwright.
