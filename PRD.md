...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, and the earlier post-completion UI refactor program remains complete through `UI-13`. The post-completion MUI refactor program is now complete through `MUI-09` on `feature/mui-refactor-foundation`: the `/sources` discovery workspace and CSV import workspace now render through the shared MUI primitives while preserving saved-search re-entry, translated outbound query visibility, duplicate-aware import preview, source lineage, and guarded CSV mapping plus preview behavior. The compose test stack also now uses a persistent non-production `test` runner plus a standalone `e2e-web` browser target so repeated lint, test, build, and Playwright verification no longer pay the earlier disposable-runner and full-runner export costs.
- Next recommended item: `MUI-10 Migrate the knowledge and tasks route families onto the MUI system.`
- Blockers: no product blocker is blocking `MUI-10`. The compose browser path now defaults to the cached `make compose-test-browser-image` rebuild that `make compose-test-e2e` invokes automatically, while `make compose-test-browser-image-fresh` remains available for explicit cache-busting or Docker-layer debugging. `make compose-test-e2e` still begins by running `docker compose down --remove-orphans`, so it can tear down a user-kept live `web`/`worker` environment; restore it afterward with `docker compose run --rm --build web npx prisma migrate deploy`, `docker compose run --rm --build web npm run db:seed`, `docker compose up --build -d web worker`, and `curl -fsS http://127.0.0.1:3000/api/health`. The compose test database still stays internal to the compose network instead of publishing host port `5432`, which avoids collisions with unrelated local PostgreSQL containers. Closing the matching GitHub issue may still fail with `403 Resource not accessible by integration`; the issue-close attempts for `#5`, `#6`, `#7`, and `#8` all hit that exact connector-permission limit, and `#9` may require the same fallback.
- Files touched in latest loop: `Dockerfile`, `Makefile`, `README.md`, `docs/testing.md`, `docker-compose.test.yml`, `next.config.ts`, `src/components/sources/source-search.tsx`, `src/components/sources/source-search.test.tsx`, `src/components/sources/csv-import-workspace.tsx`, `src/components/sources/csv-import-workspace.test.tsx`, `src/components/ui/feedback-banner.tsx`, `src/components/ui/ui-foundation.test.tsx`, `src/components/layout/authenticated-app-shell.test.tsx`, `src/components/opportunities/opportunity-list.test.tsx`, `src/components/opportunities/opportunity-proposal-manager.test.tsx`, `src/lib/tooling/compose-test-stack.test.ts`, `tests/smoke.spec.ts`, `PRD.md`, and `NOTES.md`.
- Tests run in latest loop: `npm test -- src/lib/tooling/compose-test-stack.test.ts`; `make compose-test-lint`; `make compose-test`; `make compose-test-build`; `make compose-test-e2e`; and `git diff --check`. Verification used `docker compose` and Chromium Playwright; all required checks passed for `MUI-09`. `make clean-dev-artifacts` remains intentionally skipped because the user asked to keep the environment available on the feature branch for continued work and to continue directly into the next MUI checklist item.
