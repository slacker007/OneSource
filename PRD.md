...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, and the earlier post-completion UI refactor program remains complete through `UI-13`. The post-completion MUI refactor program is now complete through `MUI-05` on `feature/mui-refactor-foundation`: shared MUI-backed `StatePanel`, `LoadingSurface`, and `ActionFeedback` primitives now drive the reusable empty, error, permission-denied, loading, and transient feedback paths, representative workspace surfaces plus the authenticated error boundary now consume those shared patterns instead of bespoke state containers, and the required compose-backed verification plus Chromium smoke coverage is green.
- Next recommended item: `MUI-06 Replace shared data-display primitives with MUI-native patterns, including Data Grid where appropriate.`
- Blockers: no blocker prevents starting `MUI-06`. The compose browser-path trap still applies: after route-level or smoke-spec edits, explicitly rebuild `web` and `playwright` with `docker compose -f docker-compose.test.yml build --no-cache web playwright` before rerunning `make compose-test-e2e`, otherwise Chromium can reuse a stale app bundle. The additional operational note from `MUI-03` still applies as well: the direct `docker compose -f docker-compose.test.yml run --rm --build test ...` commands and the `make compose-test-e2e` target share the default compose project name, so they can recreate `db` or tear down a user-kept live `web`/`worker` stack; if the live environment must remain available afterward, restore it with the documented default-stack bootstrap and `docker compose up --build -d web worker`. The compose test database now stays internal to the compose network instead of publishing host port `5432`, which avoids collisions with unrelated local PostgreSQL containers.
- Files touched in latest loop: `docker-compose.test.yml`, `src/app/(app)/opportunities/loading.tsx`, `src/app/forbidden/page.test.tsx`, `src/components/admin/admin-console.test.tsx`, `src/components/knowledge/knowledge-form.tsx`, `src/components/layout/app-error-boundary.tsx`, `src/components/layout/authenticated-app-shell.test.tsx`, `src/components/opportunities/opportunity-action-refresh.test.tsx`, `src/components/opportunities/opportunity-bid-decision-manager.tsx`, `src/components/opportunities/opportunity-closeout-manager.tsx`, `src/components/opportunities/opportunity-document-manager.tsx`, `src/components/opportunities/opportunity-form.tsx`, `src/components/opportunities/opportunity-form.test.tsx`, `src/components/opportunities/opportunity-list.test.tsx`, `src/components/opportunities/opportunity-milestone-manager.tsx`, `src/components/opportunities/opportunity-note-manager.tsx`, `src/components/opportunities/opportunity-proposal-manager.tsx`, `src/components/opportunities/opportunity-proposal-manager.test.tsx`, `src/components/opportunities/opportunity-stage-transition-panel.tsx`, `src/components/opportunities/opportunity-task-manager.tsx`, `src/components/opportunities/opportunity-workspace.test.tsx`, `src/components/sources/csv-import-workspace.test.tsx`, `src/components/sources/source-search.test.tsx`, `src/components/ui/action-feedback.tsx`, `src/components/ui/empty-state.tsx`, `src/components/ui/error-state.tsx`, `src/components/ui/feedback-banner.tsx`, `src/components/ui/loading-surface.tsx`, `src/components/ui/permission-denied-state.tsx`, `src/components/ui/state-panel.tsx`, `src/components/ui/ui-foundation.test.tsx`, `src/components/ui/ui-state-patterns.test.tsx`, `tests/smoke.spec.ts`, `vitest.config.ts`, `PRD.md`, `README.md`, `docs/testing.md`, and `NOTES.md`.
- Tests run in latest loop: `docker compose -f docker-compose.test.yml run --rm --build test npm run lint`; `docker compose -f docker-compose.test.yml run --rm --build test npm test`; `docker compose -f docker-compose.test.yml run --rm --build test npm run build`; `docker compose -f docker-compose.test.yml build --no-cache web playwright`; `make compose-test-e2e`; and `git diff --check`. During failure analysis before the final green pass, targeted Chromium reruns also exercised the authenticated-homepage and proposal-tracking smoke paths from `tests/smoke.spec.ts`. Verification used `docker compose` and Chromium Playwright; all required checks passed for `MUI-05`. `make clean-dev-artifacts` remains intentionally skipped because the user asked to keep the environment available on the feature branch for continued work.
