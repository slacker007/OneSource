...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, and the earlier post-completion UI refactor program remains complete through `UI-13`. The post-completion MUI refactor program is now complete through `MUI-13` on `feature/mui-refactor-foundation`: the remaining brittle browser and UI tests now target accessible names, `aria-current`, named `region`/`form`/`complementary` wrapper contracts, and explicit preview-panel labels instead of generated MUI classes, unlabeled `aside`/`section`/`form` lookups, or fallback `article, tr` DOM-shape selectors.
- Next recommended item: `MUI-14 Run the final full regression, accessibility and responsiveness polish, and durable doc refresh for the MUI cutover.`
- Blockers: no product blocker is blocking `MUI-14`. `make compose-test-e2e` still begins by running `docker compose down --remove-orphans`, so it can tear down a user-kept live `web`/`worker` environment; restore it afterward with `docker compose run --rm --build web npx prisma migrate deploy`, `docker compose run --rm --build web npm run db:seed`, `docker compose up --build -d web worker`, and `curl -fsS http://127.0.0.1:3000/api/health`. The compose test database still stays internal to the compose network instead of publishing host port `5432`, which avoids collisions with unrelated local PostgreSQL containers. Closing the matching GitHub issue may still fail with `403 Resource not accessible by integration`; issue-close attempts for `#5` through `#12` have all hit that exact connector-permission limit, so `#13` may require the same fallback.
- Files touched in latest loop: `src/components/ui/preview-panel.tsx`, `src/components/knowledge/knowledge-library.tsx`, `src/components/opportunities/opportunity-list.tsx`, `src/components/tasks/personal-task-board.tsx`, `src/components/sources/source-search.tsx`, `src/components/layout/authenticated-app-shell.tsx`, `src/components/opportunities/opportunity-milestone-manager.tsx`, `src/components/opportunities/opportunity-task-manager.tsx`, `src/components/ui/ui-state-patterns.test.tsx`, `src/components/ui/ui-interactions.test.tsx`, `src/components/knowledge/knowledge-library.test.tsx`, `src/components/opportunities/opportunity-list.test.tsx`, `src/components/layout/authenticated-app-shell.test.tsx`, `src/components/opportunities/opportunity-action-refresh.test.tsx`, `tests/smoke.spec.ts`, `README.md`, `docs/testing.md`, `PRD.md`, and `NOTES.md`.
- Tests run in latest loop: focused `npx vitest run src/components/ui/ui-state-patterns.test.tsx src/components/ui/ui-interactions.test.tsx src/components/knowledge/knowledge-library.test.tsx src/components/opportunities/opportunity-list.test.tsx --coverage=false`; focused `npx vitest run src/components/ui/ui-state-patterns.test.tsx src/components/ui/ui-interactions.test.tsx src/components/knowledge/knowledge-library.test.tsx src/components/opportunities/opportunity-list.test.tsx src/components/layout/authenticated-app-shell.test.tsx src/components/opportunities/opportunity-action-refresh.test.tsx --coverage=false`; `make compose-test-lint`; `make compose-test`; `make compose-test-build`; `make compose-test-e2e`; `git diff --check`; and the documented default-stack restore commands (`docker compose run --rm --build web npx prisma migrate deploy`; `docker compose run --rm --build web npm run db:seed`; `docker compose up --build -d web worker`; `curl -fsS http://127.0.0.1:3000/api/health`). Verification used `docker compose` and Chromium Playwright; all required checks passed for `MUI-13`. `make clean-dev-artifacts` remains intentionally skipped because the user asked to keep the environment available on the feature branch for continued work and to continue directly into the next MUI checklist item.
