...
- [ ] Skip that cleanup only when the user explicitly asks to keep the environment running or when preserving local artifacts is necessary to debug an unresolved blocker; if skipped or if cleanup fails, record the exact reason in `Current Handoff`.
...
- [ ] `Current Handoff` records whether `make clean-dev-artifacts` ran at loop end or exactly why it was skipped.
...
- [x] P7-01 Implement CSV import for opportunities with preview, mapping, validation, and deduplication.
...
- [x] P7-02 Add document upload support with stored metadata, linked opportunity records, and extracted plain text.
...
- [x] P7-04 Implement deduplication and canonicalization rules for multi-source opportunities.
...
- [x] P7-05 Add background jobs for scheduled ingestion, document parsing retries, and score recalculation after imports.
...
- [x] P7-06 Add source sync observability for connector health, last successful sync, rate-limit handling, and failed import review.
...
- [x] P8-01 Build a knowledge asset model and CRUD UI for past-performance snippets, boilerplate content, and win themes.
...
- [x] P8-02 Implement tagging and retrieval by agency, capability, contract type, and vehicle.
      Done when: relevant assets can be filtered down quickly in the UI.
      Verify with: query tests on seeded tagged content.

- [x] P8-03 Surface suggested knowledge assets inside the opportunity workspace based on current opportunity metadata.
...
- [x] P9-01 Build pipeline analytics for stage counts, conversion rates, aging, and upcoming deadlines.
...
- [x] P9-02 Build decision analytics for bid volume, go/no-go outcomes, score distribution, and effort versus outcome.
...
- [x] P9-03 Add win/loss outcome capture and postmortem fields.
...
- [x] P9-04 Add an admin recalibration workflow for updating scoring weights based on observed outcomes.
...
- [x] P10-01 Add a lightweight proposal record with status, owner, compliance checklist, and linked documents.
...
- [x] P10-02 Add integration boundaries for CRM export or sync, document repositories, and communication tools.
...
- [x] P10-03 Add observability and operational safeguards including structured logs, error boundaries, and health checks.
...
- [x] P10-04 Perform end-to-end launch hardening: permissions review, seed reset flow, empty/error state review, and deployment docs.
...
- [x] MVP Gate: Phases 0 through 7 are complete and the system supports real opportunity intake, scoring, workspace execution, and dashboards.
- [x] Beta Gate: Phases 8 and 9 are complete and the system supports knowledge reuse plus feedback loops.
- [x] Pilot Gate: Phase 10 is complete and the app is ready for a real internal team trial.
...
- [x] FP-01 Run one credentialed live `sam.gov` `/sources` search-and-import verification flow against the real upstream API using a configured `SAM_GOV_API_KEY`.
...
- Every user-facing item must add or update automated tests and pass Playwright verification against a live app instance before it can be marked complete.
- Use `docs/ui-refactor-plan.md` as the companion implementation guide for component inventory, route scope, data dependencies, rollout order, and testing expectations. If implementation reality diverges from that guide, update it in the same loop as the code change.

### UI Refactor Phase 1: Design System And Shared Interaction Foundation

- [x] UI-01 Harden the design system tokens, density model, status semantics, and shared surface hierarchy for a denser enterprise command-center aesthetic.
      Done when: `src/app/globals.css` and the shared UI primitives expose a disciplined typography scale, reduced corner radius, semantic status tokens, consistent borders, compact spacing defaults, and standardized skeleton, empty, error, and permission-denied treatments without removing existing accessibility support.
      Verify with: focused component tests for the updated shared primitives, full `npm run lint`, full `npm test`, `npm run build`, and one Playwright verification flow covering at least one already-refactored authenticated route on a live app instance.

- [x] UI-02 Build the reusable interaction primitives required by the redesign, including the upgraded data table, filter-chip bar, saved-view controls, preview panel or drawer pattern, density toggle, and loading skeleton patterns.
      Done when: opportunities, sources, knowledge, analytics, and admin surfaces can share one enterprise-grade interaction kit instead of page-specific one-offs, and the shared primitives support sticky headers, visible sort state, row selection scaffolding, keyboard navigation, and responsive preview behavior.
      Verify with: shared primitive tests plus at least one integration test per reused primitive, followed by full `npm run lint`, full `npm test`, `npm run build`, and Playwright verification of one table surface and one preview-panel surface.

### UI Refactor Phase 2: Shell, Navigation, And Global Search

- [x] UI-03 Refactor the authenticated application shell into grouped navigation with stronger orientation, quick links, and responsive collapsed behavior.
      Done when: the left rail is grouped by user intent, active state is clearer, pinned or recent work can be surfaced without ad hoc page links, and the shell remains usable on desktop, tablet, and mobile without regressing permissions or route guards.
      Verify with: shell component tests, route smoke tests across the authenticated app tree, full `npm run lint`, full `npm test`, `npm run build`, and Playwright desktop plus narrow-viewport navigation coverage against the live app.

- [x] UI-04 Replace the global search placeholder with a real command-and-search surface including quick-create, recent items, pinned pursuits or views, and notifications entry points.
      Done when: users can launch a keyboard-accessible command bar, jump to core entities, see recent work, and reach high-value alerts without leaving the shell, while server-side permission checks still govern any resulting navigation or actions.
      Verify with: unit and component tests for command-surface state plus data shaping, full `npm run lint`, full `npm test`, `npm run build`, and Playwright verification of command launch, keyboard navigation, and a permitted quick-action flow.

### UI Refactor Phase 3: Dashboard, Opportunities, Workspace, And External Discovery

- [x] UI-05 Redesign the dashboard into an attention-driven command center with actionable panels instead of evenly weighted metric cards.
      Done when: the dashboard highlights the attention queue, top pursuits, upcoming deadlines, pipeline risk, task burden, and recent source activity with a clear hierarchy, while all existing dashboard data remains truthful and linked to the relevant downstream views.
      Verify with: dashboard component and repository tests, full `npm run lint`, full `npm test`, `npm run build`, and Playwright verification of the main dashboard workflow plus linked navigation to at least one downstream surface.

- [x] UI-06 Redesign the opportunities list into a dense list-detail workflow with active filter chips, a filter rail or drawer, saved views, density modes, and a preview-first interaction model.
      Done when: the opportunities route supports faster scanning, server-backed sorting, result counts, saved named views, sticky headers, preview-first row selection, and an explicit `Open workspace` action without regressing existing filtering, pagination, permissions, or edit flows.
      Verify with: list component tests, query-state and sorting tests, full `npm run lint`, full `npm test`, `npm run build`, and Playwright verification of filters, saved views, preview selection, and workspace navigation on a live app instance.

- [x] UI-07 Restructure the opportunity detail page into a true workspace with a persistent header, local sub-navigation, and focused content zones for Summary, Capture, Tasks, Documents, Notes, Proposal, and History.
      Done when: the workspace stops behaving like one long-scroll report, orientation is preserved through sticky local navigation, and every existing workflow in the workspace remains functional inside the new structure, including task, milestone, note, document, proposal, decision, stage-transition, and closeout actions.
      Verify with: workspace component tests plus write-path regression tests, full `npm run lint`, full `npm test`, `npm run build`, and Playwright verification that exercises the refactored workspace tabs or sections and their critical actions on a live app instance.

- [x] UI-08 Redesign the external search and import experience into a discovery workspace with connector selection, saved searches, collapsible advanced filters, dense result scanning, translated-query visibility, inline duplicate status, and import preview.
      Done when: `/sources` clearly separates external discovery from internal tracking, advanced filters no longer dominate the page by default, saved search behavior is visible, and the create-or-link import flow remains fully functional with auditability and duplicate detection intact.
      Verify with: source-search component tests, connector translation and import-action regression tests, full `npm run lint`, full `npm test`, `npm run build`, and Playwright verification of search, preview, duplicate review, and import on a live app instance.

### UI Refactor Phase 4: Knowledge And Task Execution Surfaces

- [x] UI-09 Redesign the task surfaces for triage-first execution across `My Tasks`, `Team Tasks`, `Calendar`, and `Kanban` views while preserving task CRUD, reminder state, and opportunity linkage.
      Done when: task work is organized around priority and due-window triage rather than a single linear list, existing reminder and overdue behavior remains visible, and task actions still respect current permissions and audit requirements.
      Verify with: task-board component tests, task-service regression tests, full `npm run lint`, full `npm test`, `npm run build`, and Playwright verification of at least personal-task triage plus one secondary task view on the live app.

- [x] UI-10 Redesign the knowledge library into a preview-first strategic asset browser with stronger taxonomy filters, linked-pursuit visibility, and future-friendly quick-insert or copy affordances.
      Done when: knowledge assets can be scanned, filtered, previewed, and edited from a denser operational layout, linked agencies, capabilities, vehicles, and opportunities are easier to see, and current CRUD behavior remains fully functional.
      Verify with: knowledge-library component tests, knowledge write-path regression tests, full `npm run lint`, full `npm test`, `npm run build`, and Playwright verification of browse, filter, preview, and create or edit flows on a live app instance.

### UI Refactor Phase 5: Analytics, Admin, And Full Hardening

- [x] UI-11 Redesign the analytics surfaces into comparison-first portfolio views with drill-through to the underlying opportunities and reduced decorative framing.
      Done when: ranking, score distribution, funnel, stage-aging, effort-versus-outcome, and decision-alignment modules are easier to compare, visual density supports expert scanning, and users can reach the relevant opportunities from the analytics context.
      Verify with: analytics component and repository tests, full `npm run lint`, full `npm test`, `npm run build`, and Playwright verification of the decision console, filters, and drill-through behavior on the live app.

- [x] UI-12 Redesign the admin and settings surfaces into dense operator workspaces for Workspace, Users & Roles, Connectors, Saved Searches, Audit, and Scoring Profile.
      Done when: admin pages stop reading like a developer console, connector health and sync state are easier to scan, visible copy is product-facing, and the existing permission model still blocks unauthorized access to admin features.
      Verify with: admin component tests, permission regression tests, full `npm run lint`, full `npm test`, `npm run build`, and Playwright verification of admin access, viewer denial, and connector or scoring-profile inspection on the live app.

- [x] UI-13 Run full UI refactor hardening across every major route: accessibility, responsiveness, copy cleanup, performance smoothing, regression expansion, and durable doc refresh.
...
- [x] MUI-18 Rebuild the authenticated left navigation as a traditional MUI admin rail.
      Done when: the desktop shell uses a persistent MUI drawer with grouped navigation on large screens, the left navigation hierarchy stays clearer and permission-aware, the old rail workbench is removed, and pinned or recent destinations remain available only through the command center without regressing mobile navigation.
...
- [x] MUI-21 Split the remaining settings operator hub into focused admin workspaces.
      Done when: `/settings` is a lightweight workspace-admin overview, `/settings/connectors`, `/settings/saved-searches`, `/settings/scoring`, and `/settings/audit` isolate the connector, saved-search, scoring, and audit jobs from the overview route, `/settings/users` remains intact, the admin rail exposes every focused settings destination with most-specific active-route highlighting, existing retry, recalibration, invite, role-update, disable, and re-enable flows still work, and every settings route remains guarded by `manage_workspace_settings`.
      Verify with: focused admin component, admin repository, and shell-nav tests, full `npm run lint`, full `npm test`, full `npm run build`, `npm run db:seed`, Playwright Chromium verification against a temporary fixture-backed built server covering admin access, connector retry, scoring recalibration, route sweep, and viewer denial across the settings route family, and `git diff --check`.

- [x] MUI-22 Split selected-user administration into a dedicated user detail workspace.
      Done when: `/settings/users` remains a compact users-and-roles registry for scan, filter, and invite workflows; selecting a user opens `/settings/users/[userId]`; the detail route shows full identity, auth posture, role assignments, activity footprint, recent user audit context, and access-state controls; existing invite, role-update, disable, and re-enable flows still work; the admin rail keeps `Users & Roles` active for selected-user routes while the shell labels the detail route; and every route and action remains guarded by `manage_workspace_settings`.
      Verify with: focused admin user-management/detail component tests, admin repository detail read-model tests, shell active-route tests, full `npm run lint`, full `npm test`, full `npm run build`, `npm run db:seed`, Playwright Chromium verification against a temporary fixture-backed built server covering list-to-detail navigation plus role/status mutations, and `git diff --check`.

- [x] MUI-23 Polish the selected-user admin profile layout.
      Done when: `/settings/users/[userId]` preserves the selected-user detail route, read model, permissions, and audited role/status actions while rendering as a calmer admin profile workspace with a compact identity header, a crisp low-radius metric ledger, readable profile/audit dates, reduced audit metadata noise, and minimally rounded role-assignment rows.
      Verify with: focused admin user-management/detail component tests, full `npm run lint`, full `npm test`, full `npm run build`, `npx prisma migrate deploy`, `npm run db:seed`, in-app browser inspection plus Playwright Chromium verification against a temporary fixture-backed built server, and `git diff --check`.

- [x] MUI-24 Reduce shared site-wide button rounding.
      Done when: shared OneSource buttons and raw MUI buttons use a small dedicated button radius instead of the older pill treatment, while badges/chips can remain pill-shaped where they communicate status or categorization.
      Verify with: focused shared UI and theme tests, full `npm run lint`, full `npm test`, full `npm run build`, Playwright Chromium verification against a temporary fixture-backed built server, and `git diff --check`.

- [x] MUI-25 Convert selected-user audit activity into a compact DataGrid workspace.
      Done when: `/settings/users/[userId]` renders recent user audit activity through a compact MUI `DataGrid` or equivalent virtualized grid surface instead of stacked audit cards, rows stay dense and readable for larger audit histories, metadata is available through an explicit row detail/expansion affordance, and the page preserves the existing `manage_workspace_settings` guard plus current user detail read model behavior.
      Verify with: focused admin user-detail component tests covering audit rows, empty state, metadata expansion, and no raw metadata dump by default; Playwright Chromium verification against a seeded user with many audit rows such as `/settings/users/cmoc8q6j0004cemcdxyzel9v4`; full `npm run lint`; full `npm test`; full `npm run build`; `npm run db:seed`; and `git diff --check`.

- [x] MUI-26 Polish the organization audit activity workspace with a DataGrid and metadata drawer.
      Done when: `/settings/audit` renders organization-scoped audit activity through a compact MUI `DataGrid` or equivalent grid surface, the audit count no longer appears as an oversized full-width metric card, raw metadata is hidden from rows by default, a row-level action opens a left-side flyout with pretty JSON metadata and row context, and the current `manage_workspace_settings` guard plus audit read model behavior remain intact.
      Verify with: focused admin audit component and repository tests covering the compact grid, hidden raw metadata, pretty JSON metadata drawer, and full metadata read-model field; Playwright Chromium verification against `/settings/audit`; full `npm run lint`; full `npm test`; full `npm run build`; `npm run db:seed`; and `git diff --check`.
...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, the earlier post-completion UI refactor program remains complete through `UI-13`, and the post-completion MUI follow-ups remain complete through `MUI-26`. The latest loop completed GitHub issue #24 by converting `/settings/audit` from the oversized full-width audit table plus inline raw JSON into a compact MUI `DataGrid` workspace with an integrated row-count header and a left-side metadata drawer. Follow-up browser review on PR #25 also tightened the organization audit row density, replaced text metadata actions with compact icon buttons, made table/DataGrid radius use the small button token, added a shared compact `PAGE_HEADER_SURFACE_SX` token for route-level header cards, applied the same compact rhythm to the authenticated shell route header site-wide, reduced the shared panel/card radius token from `18px` to `6px`, and forced shared buttons plus button-like toggles to use explicit `6px` radius instead of MUI-scaled numeric radii.
- Next recommended item: address production-readiness issue #22, `Persist and share document upload storage across web and worker`, before pilot deployment hardening continues; then address issue #23, `Add DB-backed worker leases and current-record uniqueness guards`.
- Blockers: no product blocker is open. Two pre-existing untracked local artifacts remain intentionally untouched as user-owned or external-tool output: `.playwright-cli/` and `output/`.
- Files touched in latest loop: `NOTES.md`, `PRD.md`, `docs/architecture.md`, `docs/testing.md`, `docs/ui-refactor-plan.md`, `src/app/(app)/settings/audit/page.tsx`, `src/app/design-reset.css`, `src/components/admin/admin-audit-settings.tsx`, `src/components/admin/admin-console.tsx`, `src/components/admin/admin-console.test.tsx`, `src/components/admin/admin-user-management.test.tsx`, `src/components/admin/admin-user-detail.tsx`, `src/components/admin/admin-user-management.tsx`, `src/components/analytics/decision-console.tsx`, `src/components/auth/public-access-shell.tsx`, `src/components/home/dashboard-landing.tsx`, `src/components/knowledge/knowledge-form.tsx`, `src/components/knowledge/knowledge-library.tsx`, `src/components/layout/app-theme-provider.test.tsx`, `src/components/layout/authenticated-app-shell.tsx`, `src/components/opportunities/opportunity-form.tsx`, `src/components/opportunities/opportunity-workspace.tsx`, `src/components/sources/source-search.tsx`, `src/components/tasks/personal-task-board.tsx`, `src/components/ui/button.tsx`, `src/components/ui/data-table.tsx`, `src/components/ui/density-toggle.tsx`, `src/components/ui/feedback-banner.tsx`, `src/components/ui/page-header.ts`, `src/components/ui/skeleton.tsx`, `src/components/ui/ui-foundation.test.tsx`, `src/modules/admin/admin.repository.ts`, `src/modules/admin/admin.repository.test.ts`, `src/modules/admin/admin.types.ts`, `src/theme/onesource-theme.ts`, `src/theme/onesource-theme.test.ts`, and `tests/smoke.spec.ts`. GitHub issue created for the loop: #24 `Polish audit activity workspace with DataGrid and metadata drawer`.
- Tests run in latest loop: `npm ci`; focused `npx vitest run src/components/admin/admin-console.test.tsx src/modules/admin/admin.repository.test.ts --coverage=false` (`2` files and `11` tests passed); focused touched-file ESLint; `npm run build`; full `npm run lint`; initial full `npm test` and `npx vitest run --coverage --testTimeout=60000` attempts exposed host worker-contention timeouts in unrelated heavy UI tests, then full serial coverage passed with `npx vitest run --coverage --maxWorkers=1` (`77` files and `264` tests passed); `npx prisma migrate deploy`; repeated `npm run db:seed`; full Playwright Chromium smoke with `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3001 npm run e2e` against a temporary built server started with `SAM_GOV_USE_FIXTURES=true` passed with `9` tests after stabilizing the longer settings smoke and note-submit waits; follow-up visual-polish checks passed with `npm run lint`; `npm run build`; focused `npx vitest run src/components/ui/ui-foundation.test.tsx src/components/admin/admin-console.test.tsx src/components/admin/admin-user-management.test.tsx --coverage=false`; focused shell follow-up checks with `npx vitest run src/components/layout/authenticated-app-shell.test.tsx --coverage=false`; focused radius-token checks with `npx vitest run src/theme/onesource-theme.test.ts src/components/layout/app-theme-provider.test.tsx src/components/ui/ui-foundation.test.tsx src/components/layout/authenticated-app-shell.test.tsx --coverage=false`; `git diff --check`; and in-app browser screenshots on `/settings/audit`, `/settings/users`, `/opportunities`, and `/knowledge`. `make clean-dev-artifacts` completed for the original `MUI-26` loop and was intentionally not rerun after PR-review polish so the user's live `127.0.0.1:3000` app remains available for review. Docker Compose was not used for test execution in this loop because the user requested the faster host Node/Postgres path; Playwright Chromium and browser-use inspection were used against local app instances.
