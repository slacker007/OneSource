...
- [ ] MUI-25 Convert selected-user audit activity into a compact DataGrid workspace.
...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, the earlier post-completion UI refactor program remains complete through `UI-13`, and the post-completion MUI follow-ups remain complete through `MUI-24`. The latest loop polished the selected-user administration page after browser review and then reduced shared site-wide button rounding: `/settings/users/[userId]` now uses a compact identity/profile header, a crisp metric ledger instead of rounded stat cards, readable date labels, quieter audit metadata handling, and minimally rounded role-assignment rows; shared OneSource buttons and raw MUI buttons now use a small dedicated button radius while preserving the existing detail route, permission guard, and audited management actions.
- Next recommended item: `MUI-25 Convert selected-user audit activity into a compact DataGrid workspace`, starting with the high-activity seeded route `/settings/users/cmoc8q6j0004cemcdxyzel9v4`; after that, create the GitHub issue `Split settings into dedicated admin workspaces` once the user confirms the action-time issue creation, then push `codex/settings-admin-route-split` and confirm the hosted GitHub Actions run.
- Blockers: no product blocker is open. GitHub issue creation is intentionally pending action-time confirmation because it posts the ticket body to GitHub. Two pre-existing untracked local artifacts remain intentionally untouched as user-owned or external-tool output: `.playwright-cli/` and `output/`.
- Files touched in latest loop: `NOTES.md`, `PRD.md`, `src/components/admin/admin-user-detail.tsx`, `src/components/admin/admin-user-management.test.tsx`, `src/components/ui/button.tsx`, `src/components/ui/ui-foundation.test.tsx`, `src/theme/onesource-theme.ts`, and `src/theme/onesource-theme.test.ts`.
- Tests run in latest loop: `npx prettier --write src/components/admin/admin-user-detail.tsx src/components/admin/admin-user-management.test.tsx src/components/ui/button.tsx src/components/ui/ui-foundation.test.tsx src/theme/onesource-theme.ts src/theme/onesource-theme.test.ts NOTES.md PRD.md`; focused `npm test -- src/components/admin/admin-user-management.test.tsx`; focused `npm test -- src/components/admin/admin-user-management.test.tsx src/components/ui/ui-foundation.test.tsx src/theme/onesource-theme.test.ts`; touched-file `./node_modules/.bin/eslint ...`; full `npm run lint`; full `npm test` (`77` files and `263` tests passed); full `npm run build`; `npx prisma migrate deploy`; `npm run db:seed`; in-app browser inspection of `http://127.0.0.1:3001/settings/users/cmoc84dcg000gem5n8m1mleuy`; full Playwright Chromium smoke with `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3001 npm run e2e` against a temporary built server started with `SAM_GOV_USE_FIXTURES=true` passed with `9` tests; `git diff --check` passed; and `make clean-dev-artifacts` completed successfully.
