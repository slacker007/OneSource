...
- Current status: `P0-01`, `P0-02`, `P0-02a`, `P0-03`, `P0-04`, `P1-01`, `P1-02`, `P1-02a`, `P1-03`, `P1-04`, `P1-05`, `P2-01`, `P2-02`, `P2-03`, `P2-04`, `P3-01`, `P3-02`, `P3-03`, `P4-01`, `P4-01a`, `P4-01b`, `P4-02`, `P4-03`, `P4-04`, `P5-01`, `P5-02`, `P5-03`, `P5-04`, `P6-01`, `P6-02`, `P6-03`, `P6-04`, `P6-05`, `P7-01`, `P7-02`, `P7-03`, `P7-04`, `P7-05`, `P7-06`, `P8-01`, `P8-02`, `P8-03`, `P9-01`, `P9-02`, `P9-03`, and `P9-04` are complete. The guarded `/settings` route now exposes an admin recalibration workflow that summarizes observed awarded, lost, and no-bid outcomes, suggests factor-weight adjustments from closed scorecard evidence, accepts manual threshold and weight changes without code edits, bumps the scoring-model version, writes an audit event, and immediately recalculates current scorecards through the existing persistence sweep. The project as a whole still cannot be claimed complete in this loop because later items `P10-01` through `P10-04` remain unchecked.
- Next recommended item: `P10-01 Add a lightweight proposal record with status, owner, compliance checklist, and linked documents.`
- Blockers: no product blocker remains on `P9-04`; the credentialed live `/sources` run remains intentionally deferred to post-project follow-on `FP-01`. Current limitation, not blocker: the decision-console effort metric intentionally uses tracked execution artifacts as a proxy because the schema does not yet persist hours or labor-cost fields. Current limitation, not blocker: scheduled sync execution is only implemented for saved searches whose `sourceSystem` is `sam_gov`, so the seeded future-connector search still logs a warning and remains non-executable until those connectors land. Current limitation, not blocker: compose builds still emit Prisma OpenSSL detection warnings inside the slim Node image even though build and runtime verification pass in this environment. Current environment limitation, not blocker: host-side `npm run e2e` against the Playwright-managed local web server hit intermittent PostgreSQL `XX000 unexpected data beyond EOF` storage errors while writing seeded source and scorecard rows on this machine; the canonical compose-managed verification path (`make compose-test-e2e`) passed cleanly after a fresh migrate-and-seed cycle. Unrelated repo sync blocker remains: the earlier local history rewrite still has not been force-pushed to `origin` from this environment because the remote is SSH-based and this runtime lacks an authenticated SSH path.
- Files touched in latest loop: `NOTES.md`, `PRD.md`, `README.md`, `docs/architecture.md`, `docs/runbook.md`, `docs/security.md`, `docs/testing.md`, `src/app/(app)/settings/actions.ts`, `src/app/(app)/settings/page.tsx`, `src/components/admin/admin-console.test.tsx`, `src/components/admin/admin-console.tsx`, `src/modules/admin/admin.repository.test.ts`, `src/modules/admin/admin.repository.ts`, `src/modules/admin/admin.types.ts`, `src/modules/admin/scoring-recalibration.service.test.ts`, `src/modules/admin/scoring-recalibration.service.ts`, `src/modules/admin/scoring-recalibration.test.ts`, `src/modules/admin/scoring-recalibration.ts`, `src/modules/audit/audit.service.ts`, `src/modules/opportunities/opportunity-scorecard-job.test.ts`, `src/modules/opportunities/opportunity-scorecard-job.ts`, and `tests/smoke.spec.ts`.
- Tests run in latest loop: full `P9-04` focused, host, and compose-backed recalibration verification. Commands run: focused `npm test -- src/modules/admin/scoring-recalibration.test.ts src/modules/admin/scoring-recalibration.service.test.ts src/modules/admin/admin.repository.test.ts src/components/admin/admin-console.test.tsx src/modules/opportunities/opportunity-scorecard-job.test.ts`; focused `npm run build`; full host `docker compose up -d db`; `npm run prisma:validate`; `npm run db:seed`; `npm run lint`; `npm test`; `npm run build`; host Playwright `npm run e2e` failed on this machine with PostgreSQL `XX000 unexpected data beyond EOF` storage errors during seeded row writes; targeted browser reruns plus the changed-area compose-backed smoke checks passed after selector corrections and a fresh migrate-and-seed cycle. Compose-managed verification passed with `make compose-test-lint`; `make compose-test`; `make compose-test-build`; `make compose-test-e2e`; plus hygiene `git diff --check` and `make compose-down`. Additional environment-triage commands run during the browser investigation: `docker compose down -v`; `rm -rf .docker/postgres-data && mkdir -p .docker/postgres-data`; `npx prisma migrate deploy`; targeted `npx playwright test tests/smoke.spec.ts --project=chromium --grep "authenticated homepage smoke test|users can open the opportunity workspace and review seeded sections|users can record closeout notes on a closed opportunity workspace|small-screen users can navigate from the shell drawer|viewer users are blocked from the restricted settings route"`; and `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 npm run e2e`. Verification used `docker compose` for the canonical browser path and the full compose lint, test, build, and Playwright runs.
