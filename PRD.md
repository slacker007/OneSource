...
- Current status: `P0-01`, `P0-02`, `P0-02a`, `P0-03`, `P0-04`, `P1-01`, `P1-02`, `P1-02a`, `P1-03`, `P1-04`, `P1-05`, `P2-01`, `P2-02`, `P2-03`, `P2-04`, `P3-01`, `P3-02`, `P3-03`, `P4-01`, `P4-01a`, `P4-01b`, and `P4-02` are complete. This loop added guarded create and edit routes for tracked opportunities, a typed form-validation boundary, server actions over the audited opportunity write service, browser-local draft autosave and restore, create/edit entry points from the opportunity list, and an idempotent Chromium flow that creates then updates a tracked opportunity.
- Next recommended item: `P4-03 Build the opportunity detail workspace with overview, scoring, tasks, documents, notes, and history sections.`
- Blockers: No product blocker for `P4-03`. Unrelated repo sync blocker remains: the earlier local history rewrite still has not been force-pushed to `origin` from this environment because the remote is SSH-based and this runtime lacks an authenticated SSH path.
- Files touched in latest loop: `NOTES.md`, `PRD.md`, `README.md`, `docs/architecture.md`, `docs/runbook.md`, `docs/security.md`, `docs/testing.md`, `src/app/(app)/opportunities/actions.ts`, `src/app/(app)/opportunities/new/page.tsx`, `src/app/(app)/opportunities/[opportunityId]/edit/page.tsx`, `src/components/opportunities/opportunity-form.tsx`, `src/components/opportunities/opportunity-form.test.tsx`, `src/components/opportunities/opportunity-list.tsx`, `src/components/opportunities/opportunity-list.test.tsx`, `src/components/ui/textarea.tsx`, `src/modules/opportunities/opportunity-form.repository.ts`, `src/modules/opportunities/opportunity-form.schema.ts`, `src/modules/opportunities/opportunity-form.schema.test.ts`, `src/modules/opportunities/opportunity.types.ts`, and `tests/smoke.spec.ts`.
- Tests run in latest loop: `npm test -- src/modules/opportunities/opportunity-form.schema.test.ts src/components/opportunities/opportunity-form.test.tsx src/components/opportunities/opportunity-list.test.tsx src/modules/opportunities/opportunity-write.service.test.ts`; `npm run lint`; `npm run build`; `docker compose up -d db`; `npm run prisma:validate`; `npm run db:seed`; `npm run lint`; `npm test`; `npm run build`; `npm run e2e`; `rm -rf .next`; `npm run e2e`; `make compose-test-lint`; `make compose-test`; `make compose-test-build`; `make compose-test-e2e`; `make compose-down`; and `git diff --check`. Verification used Docker and Playwright.
