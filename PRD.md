...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, and the earlier post-completion UI refactor program remains complete through `UI-13`. The post-completion MUI refactor program is now complete through `MUI-07` on `feature/mui-refactor-foundation`: the dashboard landing and `/analytics` decision console now render through the shared MUI foundation with `Surface`, shared MUI buttons, and MUI table treatment for comparison and summary modules, so the operator-facing homepage and analytics route no longer depend on the earlier Tailwind-first card and table shells while preserving seeded dashboard truth, ranking behavior, and analytics drill-through links. The required compose-backed verification plus Chromium smoke coverage is green.
- Next recommended item: `MUI-08 Migrate the opportunities route family, including list, forms, and workspace sections, onto the MUI system.`
- Blockers: no blocker prevents starting `MUI-08`. The compose browser-path trap still applies: after route-level or smoke-spec edits, explicitly rebuild `web` and `playwright` with `docker compose -f docker-compose.test.yml build --no-cache web playwright` before rerunning `make compose-test-e2e`, otherwise Chromium can reuse a stale app bundle. The direct `docker compose -f docker-compose.test.yml run --rm --build test ...` commands and the `make compose-test-e2e` target still share the default compose project name, so they can recreate `db` or tear down a user-kept live `web`/`worker` stack; if the live environment must remain available afterward, restore it with the documented default-stack bootstrap, `docker compose run --rm --build web npx prisma migrate deploy`, `docker compose run --rm --build web npm run db:seed`, `docker compose up --build -d web worker`, and the `/api/health` check. The compose test database still stays internal to the compose network instead of publishing host port `5432`, which avoids collisions with unrelated local PostgreSQL containers. Closing the matching GitHub issue may still fail with `403 Resource not accessible by integration`; that connector-permission limit does not block repo work.
- Files touched in latest loop: `src/components/analytics/decision-console.tsx`, `src/components/home/dashboard-landing.tsx`, `PRD.md`, `README.md`, `docs/architecture.md`, `docs/testing.md`, and `NOTES.md`.
- Tests run in latest loop: focused `npm test -- src/components/home/app-shell-preview.test.tsx src/components/analytics/decision-console.test.tsx`; local `npm run build`; `docker compose -f docker-compose.test.yml run --rm --build test npm run lint`; `docker compose -f docker-compose.test.yml run --rm --build test npm test`; `docker compose -f docker-compose.test.yml run --rm --build test npm run build`; `docker compose -f docker-compose.test.yml build --no-cache web playwright`; `make compose-test-e2e`; `git diff --check`; and the documented default-stack restore commands (`docker compose run --rm --build web npx prisma migrate deploy`; `docker compose run --rm --build web npm run db:seed`; `docker compose up --build -d web worker`; `curl -fsS http://127.0.0.1:3000/api/health`). Verification used `docker compose` and Chromium Playwright; all required checks passed for `MUI-07`. `make clean-dev-artifacts` remains intentionally skipped because the user asked to keep the environment available on the feature branch for continued work.
