...
- [ ] UI-03 Refactor the authenticated application shell into grouped navigation with stronger orientation, quick links, and responsive collapsed behavior.
...
- [ ] UI-04 Replace the global search placeholder with a real command-and-search surface including quick-create, recent items, pinned pursuits or views, and notifications entry points.
...
- [ ] UI-05 Redesign the dashboard into an attention-driven command center with actionable panels instead of evenly weighted metric cards.
...
- [ ] UI-06 Redesign the opportunities list into a dense list-detail workflow with active filter chips, a filter rail or drawer, saved views, density modes, and a preview-first interaction model.
...
- [ ] UI-07 Restructure the opportunity detail page into a true workspace with a persistent header, local sub-navigation, and focused content zones for Summary, Capture, Tasks, Documents, Notes, Proposal, and History.
...
- [ ] UI-08 Redesign the external search and import experience into a discovery workspace with connector selection, saved searches, collapsible advanced filters, dense result scanning, translated-query visibility, inline duplicate status, and import preview.
...
- [ ] UI-09 Redesign the task surfaces for triage-first execution across `My Tasks`, `Team Tasks`, `Calendar`, and `Kanban` views while preserving task CRUD, reminder state, and opportunity linkage.
...
- Current status: `P0-01` through `P10-04` remain complete, the MVP/Beta/Pilot release gates remain met, deferred follow-on `FP-01` remains complete, and `UI-01` plus `UI-02` are now complete. The shared interaction layer now includes an upgraded `DataTable` with sticky headers, visible sort-state indicators, selected-row styling, and keyboard-focusable rows; reusable `active-filter-chip-bar`, `saved-view-controls`, `density-toggle`, `preview-panel`, `table-skeleton`, and `preview-panel-skeleton` primitives; and a refactored `/opportunities` route that proves reuse through standard view presets, removable filter chips, density-aware results, query-string-backed preview selection, and a dedicated loading state. The one-time project completion marker remains the earlier `2026-04-18T17:54:41Z` emission and should not be re-emitted until the full UI refactor program is actually complete.
- Next recommended item: `UI-03 Refactor the authenticated application shell into grouped navigation with stronger orientation, quick links, and responsive collapsed behavior.`
- Blockers: no blocker currently prevents `UI-03`. Current limitation, not blocker: the decision-console effort metric intentionally uses tracked execution artifacts as a proxy because the schema does not yet persist hours or labor-cost fields. Current limitation, not blocker: scheduled sync execution is only implemented for saved searches whose `sourceSystem` is `sam_gov`, so the seeded future-connector search still logs a warning and remains non-executable until those connectors land. Current limitation, not blocker: the CRM, document-repository, and communication adapters are deterministic dry runs only, so no live outbound credentials, webhook validation, or callback signatures are in scope yet. Current environment limitation, not blocker: `make compose-up-detached` currently fails in this environment with a Docker build-context `changes out of order` error before the app containers finish building, so agents may need the existing compose-managed PostgreSQL plus host-run production-app fallback for live browser verification until that build issue is resolved. Current limitation, not blocker: compose builds still emit Prisma OpenSSL detection warnings inside the slim Node image even when build and runtime verification pass. Current environment limitation, not blocker: the serialized Playwright smoke suite mutates one shared seeded database, so reruns in the same loop should begin with `npm run db:seed` before `npm run e2e` or `make compose-test-e2e` if prior smoke scenarios already changed seeded proposal, task, or closeout records. Current environment limitation, not blocker: this machine can still surface intermittent PostgreSQL storage faults such as `55000 attempted to delete invisible tuple` or `XX000 unexpected data beyond EOF`; the verified recovery path remains `docker compose down -v --remove-orphans`, remove and recreate `.docker/postgres-data` plus `.data/opportunity-documents`, then rerun `docker compose up -d db`, `npx prisma migrate deploy`, and `npm run db:seed` before continuing verification. Unrelated repo sync blocker remains: the earlier local history rewrite still has not been force-pushed to `origin` from this environment because the remote is SSH-based and this runtime lacks an authenticated SSH path.
- Files touched in latest loop: `NOTES.md`, `PRD.md`, `README.md`, `docs/ui-refactor-plan.md`, `src/app/(app)/opportunities/loading.tsx`, `src/app/(app)/opportunities/page.tsx`, `src/components/opportunities/opportunity-list.tsx`, `src/components/opportunities/opportunity-list.test.tsx`, `src/components/ui/active-filter-chip-bar.tsx`, `src/components/ui/data-table.tsx`, `src/components/ui/density-toggle.tsx`, `src/components/ui/preview-panel.tsx`, `src/components/ui/preview-panel-skeleton.tsx`, `src/components/ui/saved-view-controls.tsx`, `src/components/ui/table-skeleton.tsx`, `src/components/ui/ui-interactions.test.tsx`, and `tests/smoke.spec.ts`.
- Tests run in latest loop: focused `npm test -- src/components/ui/ui-foundation.test.tsx src/components/ui/ui-interactions.test.tsx src/components/opportunities/opportunity-list.test.tsx`; focused `npm test -- src/components/ui/ui-interactions.test.tsx src/components/opportunities/opportunity-list.test.tsx`; focused `npm run build`; `docker compose up -d db`; `npm run prisma:validate`; attempted `npm run db:seed`; recovery `docker compose down -v --remove-orphans`; `rm -rf .docker/postgres-data .data/opportunity-documents`; `mkdir -p .docker/postgres-data .data/opportunity-documents`; `docker compose up -d db`; `npx prisma migrate deploy`; `npm run db:seed`; full `npm test`; full `npm run build`; `SAM_GOV_USE_FIXTURES=true npm run e2e`; follow-up `npm run db:seed`; recovery `docker compose down -v --remove-orphans`; `rm -rf .docker/postgres-data .data/opportunity-documents`; `mkdir -p .docker/postgres-data .data/opportunity-documents`; `docker compose up -d db`; `npx prisma migrate deploy`; `npm run db:seed`; green `SAM_GOV_USE_FIXTURES=true npm run e2e`; final full-rerun after the last table changes: `npm test`; `npm run build`; `npm run db:seed`; recovery `docker compose down -v --remove-orphans`; `rm -rf .docker/postgres-data .data/opportunity-documents`; `mkdir -p .docker/postgres-data .data/opportunity-documents`; `docker compose up -d db`; `npx prisma migrate deploy`; `npm run db:seed`; and final green `SAM_GOV_USE_FIXTURES=true npm run e2e`. `docker compose` was used for PostgreSQL, and Playwright Chromium ran against the live app on the host-managed path.
