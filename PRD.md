...
- Files touched in latest loop: `NOTES.md`, `PRD.md`, `docs/ui-refactor-plan.md`, `OneSource-UI-Secification-Guide.md` (deleted), `src/components/opportunities/opportunity-form.tsx`, `src/components/opportunities/opportunity-list.tsx`, `src/components/opportunities/opportunity-list.test.tsx`, `src/components/opportunities/opportunity-workspace.tsx`, `src/components/opportunities/opportunity-workspace.test.tsx`, and `tests/smoke.spec.ts`.
- Tests run in latest loop: focused `npx eslint src/components/opportunities/opportunity-list.tsx src/components/opportunities/opportunity-list.test.tsx`; focused `npx vitest run src/components/opportunities/opportunity-list.test.tsx`; focused `npx eslint src/components/opportunities/opportunity-form.tsx src/components/opportunities/opportunity-workspace.tsx src/components/opportunities/opportunity-form.test.tsx src/components/opportunities/opportunity-workspace.test.tsx`; focused `npx vitest run src/components/opportunities/opportunity-form.test.tsx src/components/opportunities/opportunity-workspace.test.tsx`; repeated focused `npx eslint tests/smoke.spec.ts`; preflight Playwright Chromium checks on the user-kept app via `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 npx playwright test tests/smoke.spec.ts --project=chromium -g "small-screen users can navigate from the shell drawer|major authenticated routes stay readable without horizontal overflow"`; repeated targeted Playwright Chromium reruns on the temporary built server while stabilizing the updated opportunities smoke contract; final full local `npm run lint`; full local `npm test`; full local `npm run build`; repeated `npm run db:seed`; final full local Chromium smoke via `SAM_GOV_USE_FIXTURES=true PLAYWRIGHT_BASE_URL=http://127.0.0.1:3002 npm run e2e`; and `git diff --check`. The temporary `3002` verification server was shut down after the final green browser pass; the user-kept main stack was left running.
