...
- allow the admin user through the restricted settings route family and render `/settings` as an overview, `/settings/connectors` as connector health plus sync operations, `/settings/saved-searches` as the saved-search registry, `/settings/scoring` as scoring profile plus observed-outcome recalibration, `/settings/audit` as recent audit visibility, `/settings/users` as the users-and-roles registry, and `/settings/users/[userId]` as selected-user role and access management
- execute one `Retry sync` action from `/settings/connectors` and observe the queued success notice on the reloaded page
- execute one scoring recalibration from `/settings/scoring` and observe the success notice plus updated form value
- redirect the seeded viewer user from each restricted settings route to `/forbidden`
- complete the tablet-sized route sweep for `/`, `/opportunities`, `/tasks`, `/knowledge`, `/sources`, `/analytics`, the settings route family, and one opportunity workspace while confirming `document.documentElement.scrollWidth <= window.innerWidth + 1` on each visited surface
...
- the guarded `/settings/connectors` surface renders connector health, recent source sync runs, failed import review rows, and the retry success banner without exposing raw database state
- the Playwright smoke flow can queue one retry from `/settings/connectors` and observe the queued notice after the page reloads
...
- the admin console renders the scoring-profile sections, decision-threshold badges, weighted-criteria table, observed-outcome summaries, and recalibration form on the guarded `/settings/scoring` route
