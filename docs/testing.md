...
The compose `test` service now targets a dedicated non-production Docker stage, so direct `docker compose -f docker-compose.test.yml run --rm --build test ...` commands skip the otherwise unnecessary app `next build` that the production `runner` target performs. This reduces lint, unit-test, migration, and seed startup cost. The compose browser path now also targets a dedicated standalone `e2e-web` image so Playwright reruns export a much smaller web image than the full production runner.
