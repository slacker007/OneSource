...
- Typed knowledge-library parsing, list/form snapshot mapping, and audited create/update/delete writes now live in `src/modules/knowledge`.
...
- `knowledge_asset_tags`: freeform reusable-content tags normalized into one row per label
...
- three seeded reusable knowledge assets spanning past performance, boilerplate content, and win themes, each with freeform tags and linked opportunities
...
- The shared app shell, reusable UI-pattern kit, seeded dashboard landing page, real opportunity list, real opportunity workspace, real decision console, task CRUD, milestone CRUD, guarded note creation, guarded document upload, real personal `/tasks` queue, real external-search page, and the baseline `/knowledge` library now exist, but the later connector hardening, ingestion orchestration, deeper knowledge retrieval, feedback-loop analytics, and proposal slices still need to land
