...
- The shared app shell, reusable UI-pattern kit, seeded dashboard landing page, real opportunity list, real opportunity workspace, real decision console, task CRUD, milestone CRUD, guarded note creation, guarded document upload, real personal `/tasks` queue, real external-search page, and the structured `/knowledge` library now exist, but the later in-workspace knowledge suggestions, connector hardening, ingestion orchestration, feedback-loop analytics, and proposal slices still need to land
