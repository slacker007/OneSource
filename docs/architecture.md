...
- The shared app shell, reusable UI-pattern kit, seeded dashboard landing page with pipeline-health analytics, real opportunity list, real opportunity workspace, real decision console, task CRUD, milestone CRUD, guarded note creation, guarded document upload, real personal `/tasks` queue, real external-search page, the structured `/knowledge` library, and in-workspace knowledge suggestions now exist, but the remaining decision-feedback analytics, recalibration, proposal, connector-hardening, and operational-hardening slices still need to land
