...
- The shared app shell, reusable UI-pattern kit, seeded dashboard landing page with pipeline-health analytics, real opportunity list, real opportunity workspace, real decision console plus portfolio decision analytics, closeout capture for closed pursuits, task CRUD, milestone CRUD, guarded note creation, guarded document upload, real personal `/tasks` queue, real external-search page, the structured `/knowledge` library, in-workspace knowledge suggestions, and the admin recalibration workflow now exist, but the remaining proposal, connector-hardening, and operational-hardening slices still need to land
