...
22. The guarded settings route at [src/app/(app)/settings/page.tsx](</Users/maverick/Documents/RalphLoops/OneSource/src/app/(app)/settings/page.tsx:1>) calls [requireAppPermission](/Users/maverick/Documents/RalphLoops/OneSource/src/lib/auth/authorization.ts:1), loads organization-scoped admin data plus the seeded scoring profile and source-operations snapshot through [src/modules/admin/admin.repository.ts](/Users/maverick/Documents/RalphLoops/OneSource/src/modules/admin/admin.repository.ts:1), wires the guarded retry mutation through [src/app/(app)/settings/actions.ts](</Users/maverick/Documents/RalphLoops/OneSource/src/app/(app)/settings/actions.ts:1>), and renders [AdminConsole](/Users/maverick/Documents/RalphLoops/OneSource/src/components/admin/admin-console.tsx:1) through the shared table, badge, empty-state, and error-state primitives.
...
- The shared app shell, reusable UI-pattern kit, seeded dashboard landing page with pipeline-health analytics, real opportunity list, real opportunity workspace, real decision console plus portfolio decision analytics, closeout capture for closed pursuits, task CRUD, milestone CRUD, guarded note creation, guarded document upload, real personal `/tasks` queue, real external-search page, the structured `/knowledge` library, and in-workspace knowledge suggestions now exist, but the remaining recalibration, proposal, connector-hardening, and operational-hardening slices still need to land
