...
- Typed external-search parsing, connector capability metadata, reusable `sam.gov` connector execution, persisted execution and source-record lineage, preview payload shaping, duplicate detection, and import application now live in `src/modules/source-integrations`.
...
- The `sam.gov` connector exists, but scheduled-ingestion workflows are still incomplete; the credentialed live verification run is intentionally deferred to post-project follow-on `FP-01`
