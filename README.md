...
- The next recommended item is `P8-03`. The credentialed live `sam.gov` run remains intentionally deferred to post-project follow-on `FP-01`.
...
- The authenticated shell, shared UI-pattern kit, seeded dashboard landing page, real opportunity list, real opportunity workspace, decision console, task CRUD, personal `/tasks` queue, tracked-opportunity create/edit forms, stage-transition controls, real external-search page, `/settings` admin console, and the structured `/knowledge` library now exist, but the remaining in-workspace knowledge suggestions, analytics-feedback, and proposal slices still need to land behind those routes. The credentialed live `sam.gov` run is intentionally deferred to post-project follow-on `FP-01`.
