...
- `P0-01`, `P0-02`, `P0-02a`, `P0-03`, `P0-04`, `P1-01`, `P1-02`, `P1-02a`, `P1-03`, `P1-04`, `P1-05`, `P2-01`, `P2-02`, `P2-03`, `P2-04`, `P3-01`, `P3-02`, `P3-03`, `P4-01`, `P4-01a`, `P4-01b`, `P4-02`, `P4-03`, `P4-04`, `P5-01`, `P5-02`, and `P5-03` are complete.
- The next recommended item is `P5-04`, which adds deadline reminders and overdue indicators using a background job.
...
The typed opportunity module under `src/modules/opportunities/` now exposes shared DTOs, dashboard and list read models, a dedicated opportunity-workspace read model, task-assignee and personal-task-board snapshots, a stage-policy boundary with required-field gating, form-validation boundaries for opportunities, tasks, milestones, and notes, browser-draft helpers, and audited write-service integration for guarded create, edit, stage-transition, task, milestone, and note flows. The typed source-integration module under `src/modules/source-integrations/` now owns canonical external-search query parsing, `sam.gov` request translation, deterministic mocked search responses, source-result preview payload shaping, duplicate detection against existing opportunities, and guarded create-versus-link import application for Phase 4. The typed admin repository under `src/modules/admin/` exposes organization-scoped user-role visibility plus recent audit-log inspection for the guarded `/settings` route. The authenticated `(app)` route group now renders through a shared shell with a desktop sidebar, sticky top bar, read-only global search placeholder, and reusable mobile navigation drawer. The `/` dashboard route now renders a real landing page with tracked and active pursuit counts, stage distribution, upcoming deadlines, and ranked opportunities from seeded data. The `/opportunities` route now renders a real opportunity pipeline page with keyword search, agency/NAICS/stage/source/due-date filters, sort controls, pagination, URL-synced query state, a create entry point, and workspace/edit entry links; `/opportunities/[opportunityId]` now provides a real opportunity workspace with overview, scoring, task CRUD, milestone CRUD, documents, notes, history, guarded note creation, and a guarded stage-transition panel that records rationale, blocks invalid moves, and appends activity plus audit evidence; `/opportunities/new` plus `/opportunities/[opportunityId]/edit` provide guarded tracked-opportunity forms with server validation and browser-local draft restore. The `/tasks` route now hosts a real personal execution queue that surfaces assigned tasks with opportunity linkage, while `/analytics` remains a protected placeholder for later phase work.
