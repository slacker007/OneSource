...
- The post-completion Material UI program is complete through `MUI-18`. The design-reset follow-up is now active: the shared theme, dashboard, and authenticated shell use the new compact neutral baseline, the shell now follows a mini-first admin-rail pattern, and later route-family slices will keep removing remaining hybrid layout composition.
